            ...
            <div class="row justify-content-center">
                <div class="col-md-8">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <form method="POST" action="{{ route('mascotas.update', $mascota->id) }}">                                @csrf
                                @method('PUT')
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="tipo" id="tipo" class="form-control" required>
                                        @foreach(\App\Models\Mascota::TIPOS as $tipo)
                                            <option value="{{ $tipo }}" {{ $mascota->tipo == $tipo ? 'selected' : '' }}>{{ $tipo }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="form-group">
                                    ...
                                    <select name="cliente_id" id="cliente_id" class="form-control" required>
                                        @foreach ($clientes as $cliente)
                                            <option value="{{ $cliente->id }}" {{ $mascota->cliente_id == $cliente->id ? 'selected' : '' }}>{{ $cliente->nombres }}</option>
                                        @endforeach
                                    </select>
                                </div>
